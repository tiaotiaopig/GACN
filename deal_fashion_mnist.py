...
def load_train_images(dataset):
    if dataset == 'fashion':
    # 训练集文件
        idx_ubyte_file = r'train-images-idx3-ubyte'
    if dataset == 'number':
        idx_ubyte_file = r'train-images.idx3-ubyte'
...
def load_train_labels(dataset):
    if dataset == 'fashion':
        idx_ubyte_file = r'train-labels-idx1-ubyte'
    if dataset == 'number':
        idx_ubyte_file = r'train-labels.idx1-ubyte'
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if random.random() >=0.1:
            i += 1
            continue
        if train_labels[i] not in label_set_showed:
        ...
        # plt.pause(0.000001)
            plt.show()
            plt.cla()
            label_set_showed.add(train_labels[i])
            if len(label_set_showed) == len(label_set):
                break
        i += 1
    print('done')
